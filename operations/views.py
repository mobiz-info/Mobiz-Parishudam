...
def expense_create_view(request):
    ...
    if request.method == 'POST':
        ...
        head_id = request.POST.get('expense_head_id')
        expense_head = get_object_or_404(ExpenseHead, id=head_id)
        ...
        messages.success(request, f"Expense ₹{amount} for {expense_head.name} recorded successfully.")
        return redirect('expense_list')

    today_str = timezone.now().date().strftime('%Y-%m-%d')
    default_branch = user_profile.branch if user_profile and user_profile.branch else branches.first()
...
def expense_list_view(request):
    ...
    expense_qs = Expense.objects.select_related('branch', 'staff', 'expense_head').all()

    if not is_admin and user_profile and user_profile.branch:
        expense_qs = expense_qs.filter(branch=user_profile.branch)
    else:
        branch_id = request.GET.get('branch_id')
        if branch_id:
